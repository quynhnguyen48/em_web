...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      axios.get('https://api.echomedi.com/api/product/getCart',
        {
          headers: {
  ...
  const getVNPayPaymentURL = () => {
    ...
    const token = localStorage.getItem('token');
    if (token) {
      axios.post('https://api.echomedi.com/api/orders/createPaymentUrl', {},
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const createOrder = () => {
    ...
    const token = localStorage.getItem('token');
    if (token) {
      axios.post('https://api.echomedi.com/api/orders/createOrder', {},
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  return (
    <>
      ...
      image_url="https://api.echomedi.com/uploads/dat_lich_kham_5246f10577.jpg?updated_at=2023-01-07T01:43:41.793Z" message={""} sub_message={[]}/>
      ...
      <div className="w-4/4 sm:w-3/4 bg-white px-10 py-10">
        ...
        <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
          <div className="flex w-4/5">
            <div className="w-20">
              <img className="h-24 object-contain" src={"https://api.echomedi.com" + line.product?.image?.url} alt=""/>
            </div>
            <div className="flex flex-col justify-between ml-4 flex-grow">
              ...
              <button 
                onClick={() => {
                  axios.delete('https://api.echomedi.com/api/cart-lines/' + line.id, {},
                  )
                  .then(function (response) {
                    location.reload();
                  })
